...
DOMBasedOpenRedirection = Flask(__name__)
DOMBasedOpenRedirection.secret_key = "vulnerable_lab_by_IHA089"
...
@DOMBasedOpenRedirection.route('/')
def home():
    if not check_cookies():
        session.clear()
    return render_template('index.html', user=session.get('user'))

@DOMBasedOpenRedirection.route('/index.html')
def index_html():
    if not check_cookies():
        session.clear()
    return render_template('index.html', user=session.get('user'))

@DOMBasedOpenRedirection.route('/login.html')
def login_html():
    if not check_cookies():
        session.clear()
    return render_template('login.html')

@DOMBasedOpenRedirection.route('/join.html')
def join_html():
    if not check_cookies():
        session.clear()
    return render_template('join.html')

@DOMBasedOpenRedirection.route('/forgot-password.html')
...
@DOMBasedOpenRedirection.route('/confirm', methods=['POST'])
def confirm():
...
@DOMBasedOpenRedirection.route('/resend', methods=['POST'])
def resend():
    username = request.form.get('username')
    password = request.form.get('password')
    hash_password=hashlib.md5(password.encode()).hexdigest()
...
@DOMBasedOpenRedirection.route('/login', methods=['GET', 'POST'])
def login():
...
@DOMBasedOpenRedirection.route('/check', methods=['POST'])
def check():
    session_code = request.form.get('redirectionurl')
    
    if "https://iha089-labs.in/dashboard.html?next=https://iha089-labs.in@" in session_code or "https://iha089-labs.in/dashboard.html?next=https://iha089-labs.in@" in session_code or "https://iha089-labs.in/profile.html?next=https://iha089-labs.in@" in session_code or "https://iha089-labs.in/profile?next=https://iha089-labs.in@" in session_code: 
...
@DOMBasedOpenRedirection.route('/resetpassword', methods=['POST'])
def resetpassword():
    password=request.form.get('password')
...
@DOMBasedOpenRedirection.route('/join', methods=['GET', 'POST'])
def join():
    if not check_cookies():
        session.clear()
    if 'user' in session:
    ...
    try:
        ...
        mail_server = "https://127.0.0.1:7089/dcb8df93f8885473ad69681e82c423163edca1b13cf2f4c39c1956b4d32b4275"
        payload = {"email": username,
                    "sender":"IHA089 Labs ::: DOMBasedOpenRedirection",
                    "subject":"DOMBasedOpenRedirection::Verify Your Accout",
                    "bodycontent":bdcontent
                }
        try:
            k = requests.post(mail_server, json = payload, verify=False)
...
@DOMBasedOpenRedirection.route('/reset', methods=['GET'])
def reset():
...
@DOMBasedOpenRedirection.route('/forgot', methods=['POST'])
def forgot():
    try:
        ...
        if 'username' in data:
            ...
            if result is not None:
                ...
                payload = {"email": username,
                            "sender":"IHA089 Labs ::: DOMBasedOpenRedirection",
                            "subject":"DOMBasedOpenRedirection::Click bellow link to reset your password",
                            "bodycontent":bdcontent
                    }
                try:
                    k = requests.post(mail_server, json = payload, verify=False)
...
@DOMBasedOpenRedirection.route('/dashboard')
@DOMBasedOpenRedirection.route("/dashboard.html")
@login_required
def dashboard():
    if not check_cookies():
        session.clear()
...
@DOMBasedOpenRedirection.route('/logout.html')
def logout():
    session.clear() 
...
@DOMBasedOpenRedirection.route('/profile')
@DOMBasedOpenRedirection.route('/profile.html')
@login_required
def profile():
    if not check_cookies():
        session.clear()
...
@DOMBasedOpenRedirection.route('/reset_password', methods=['POST'])
def reset_password():
    username = request.form.get('username')
...
@DOMBasedOpenRedirection.after_request
